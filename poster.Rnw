...
\usepackage[scale=1.24,orientation=potrait,size=a0,debug]{beamerposter} % Use the beamerposter package for laying out the poster
...
\newlength{\twocolwid}
\newlength{\threecolwid}
\setlength{\paperwidth}{65cm} % A0 width: 46.8in
...
\setlength{\threecolwid}{0.68\paperwidth} % Width of three columns
...
\title{Epidemics on Time-Varying Empirical Networks} % Poster title
\author{Carl~A.~B.~Pearson, Thomas~J.~Hladish}
\institute{Emerging Pathogens Institute, University of Florida, Gainesville, FL} % Institution(s)
...
Contact networks are intrinsically temporal, but we often analyze them as aggregated over time.  This simplifies both analytical and simulation approaches, but with simulation approaches we may dis-aggregate with minimal additional complexity when the simulation is based on strictly empirical networks.
...
Should describe the data set here.  Collection times, number of data points, assorted other counts, etc.
...
%Comparison of the epidemic final size distribution and time evolution across some parameters, and in comparison to the same, with the same parameters, for the fully aggregated network.
